namespace gma_news_api.Infrastructure.Databases.MoviesReviews.Extensions;
...
internal static class MovieReviewsDbContextExtensions
{
    public static MovieReviewsDbContext AddData(this MovieReviewsDbContext context)
    {
        ...
        var movies = new Faker<Movie>()
            ...
            .RuleFor(m => m.DateCreated, f => f.Date.Past().ToUniversalTime())
            .RuleFor(m => m.DateModified, f => f.Date.Past().ToUniversalTime())
            .Generate(50);
        ...
        var reviews = new Faker<Review>()
            ...
            .RuleFor(r => r.DateCreated, f => f.Date.Past().ToUniversalTime())
            .RuleFor(r => r.DateModified, f => f.Date.Past().ToUniversalTime())
            .Generate(150);

        context.AddRange(reviews);

        var news = new Faker<News>()
            .RuleFor(n => n.Id, _ => Guid.NewGuid())
            .RuleFor(n => n.Title, f => f.Lorem.Sentence())
            .RuleFor(n => n.NewsUrl, f => new Uri(f.Internet.Url()))
            .RuleFor(n => n.ImageUrl, f => new Uri(f.Internet.Url()))
            .RuleFor(n => n.Section, f => f.Lorem.Word())
            .RuleFor(n => n.SubSection, f => f.Lorem.Word())
            .RuleFor(n => n.DateTimeUploaded, f => f.Date.Past().ToUniversalTime())
            .Generate(50);
